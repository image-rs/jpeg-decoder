...
impl Upsampler {
    ...
    pub fn upsample_and_interleave_row(&self, component_data: &[Vec<u16>], row: usize, output_width: usize, output: &mut [u16]) {
        let component_count = component_data.len();
        let mut line_buffer = vec![0u16; self.line_buffer_size];
...
impl Upsample for UpsamplerH2V1 {
    fn upsample_row(&self,
        ...
        output[0] = input[0];
        output[1] = ((input[0] as u32 * 3 + input[1] as u32 + 2) >> 2) as u16;

        for i in 1 .. input_width - 1 {
            let sample = 3 * input[i] as u32 + 2;
            output[i * 2]     = ((sample + input[i - 1] as u32) >> 2) as u16;
            output[i * 2 + 1] = ((sample + input[i + 1] as u32) >> 2) as u16;
        }

        output[(input_width - 1) * 2] = ((input[input_width - 1] as u32 * 3 + input[input_width - 2] as u32 + 2) >> 2) as u16;
...
impl Upsample for UpsamplerH1V2 {
    fn upsample_row(&self,
        ...
        for i in 0 .. output_width {
            output[i] = ((3 * input_near[i] as u32 + input_far[i] as u32 + 2) >> 2) as u16;
        }
    }
}
...
impl Upsample for UpsamplerH2V2 {
    fn upsample_row(&self,
        ...
        if input_width == 1 {
            let value = ((3 * input_near[0] as u32 + input_far[0] as u32 + 2) >> 2) as u16;
            output[0] = value;
            output[1] = value;
            return;
        }
        ...
        output[0] = ((t1 + 2) >> 2) as u16;

        for i in 1 .. input_width {
            let t0 = t1;
            t1 = 3 * input_near[i] as u32 + input_far[i] as u32;

            output[i * 2 - 1] = ((3 * t0 + t1 + 8) >> 4) as u16;
            output[i * 2]     = ((3 * t1 + t0 + 8) >> 4) as u16;
        }

        output[input_width * 2 - 1] = ((t1 + 2) >> 2) as u16;
    }
}
