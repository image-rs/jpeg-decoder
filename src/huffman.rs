...
pub fn fill_default_mjpeg_tables(
    ...
    if dc_huffman_tables[0].is_none() && scan.dc_table_indices.iter().any(|&i| i == 0) {
        // Table K.3
        #[rustfmt::skip]
        const K3_BITS: [u8; 16] = [0x00, 0x01, 0x05, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];

        #[rustfmt::skip]
        const K3: &[u8] = &[0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B];

        dc_huffman_tables[0] =
            Some(HuffmanTable::new(&K3_BITS, K3, HuffmanTableClass::DC).unwrap());
    }

    if dc_huffman_tables[1].is_none() && scan.dc_table_indices.iter().any(|&i| i == 1) {
        // Table K.4
        #[rustfmt::skip]
        const K4_BITS: [u8; 16] = [0x00, 0x03, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00];

        #[rustfmt::skip]
        const K4: &[u8] = &[0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B];

        dc_huffman_tables[1] =
            Some(HuffmanTable::new(&K4_BITS, K4, HuffmanTableClass::DC).unwrap());
    }

    if ac_huffman_tables[0].is_none() && scan.ac_table_indices.iter().any(|&i| i == 0) {
        // Table K.5
        #[rustfmt::skip]
        const K5_BITS: [u8; 16] = [0x00, 0x02, 0x01, 0x03, 0x03, 0x02, 0x04, 0x03, 0x05, 0x05, 0x04, 0x04, 0x00, 0x00, 0x01, 0x7D];
        ...
        ac_huffman_tables[0] =
            Some(HuffmanTable::new(&K5_BITS, K5, HuffmanTableClass::AC).unwrap());
    }

    if ac_huffman_tables[1].is_none() && scan.ac_table_indices.iter().any(|&i| i == 1) {
        // Table K.6
        #[rustfmt::skip]
        const K6_BITS: [u8; 16] = [0x00, 0x02, 0x01, 0x02, 0x04, 0x04, 0x03, 0x04, 0x07, 0x05, 0x04, 0x04, 0x00, 0x01, 0x02, 0x77];
        ...
        ac_huffman_tables[1] =
            Some(HuffmanTable::new(&K6_BITS, K6, HuffmanTableClass::AC).unwrap());
    }
}
