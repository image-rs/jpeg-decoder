...
// This is a rust translation of stb_image's 'stbi__idct_block'.
pub fn idct(coefficients: &[i32; 64], output: &mut [u8; 64]) {
    let mut temp = [0; 64];
    ...
    for i in 0 .. 8 {
        ...
        if coefficients[i + 8] == 0 && coefficients[i + 16] == 0 && coefficients[i + 24] == 0 &&
                ...
                coefficients[i + 56] == 0 {
            let dcterm = coefficients[i].wrapping_shl(2);
            temp[i]      = dcterm;
            temp[i + 8]  = dcterm;
            temp[i + 16] = dcterm;
            temp[i + 24] = dcterm;
            ...
            let s0 = coefficients[i];
            let s1 = coefficients[i + 8];
            let s2 = coefficients[i + 16];
            let s3 = coefficients[i + 24];
            let s4 = coefficients[i + 32];
            let s5 = coefficients[i + 40];
            let s6 = coefficients[i + 48];
            let s7 = coefficients[i + 56];
