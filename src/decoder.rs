                ...
                for (i, component) in components.iter().enumerate() {
                    for j in 0..blocks_per_mcu[i] {
                        let (block_x, block_y) = if is_interleaved {
                            ...
                            if x * component.dct_scale as u16 >= component.size.width
                                || y * component.dct_scale as u16 >= component.size.height
                            {
                                continue;
                            }
        ...
        4 => {
            ...
            match color_transform {
                ...
                None => Err(Error::Format(
                    "4 components without Adobe APP14 metadata to tell color space".to_owned(),
                )),
            }
        }
        _ => panic!(),
